...
import { updateDropShadow } from "../dropShadowSlice"
import ReactSlider from "react-slider"

const Sliders = () => {
  const dispatch = useAppDispatch()
  const { dropShadowData, currentTab } = useAppSelector(state => state.dropShadow)
  ...
  return (
    <div>
      {dropShadowData.map((dropShadow, index) => (
        <div key={index}>
          {currentTab === index &&
            Object.entries(dropShadow.settings).map((property, i) => {
                  ...
                  <div key={i} className="menuBlock m-2 h-8 min-w-max p-2 pb-14 ">
                    ...
                    <div className=" w-fill h-[20px] py-2">
                      <ReactSlider
                        ...
                        value={property[1]}
                        onChange={value =>
                          dispatch(updateDropShadow({ value: value, name: property[0] }))
                        }
                      />
                    </div>
                  </div>
